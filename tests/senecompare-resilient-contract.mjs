  ...
  ASSETS: {
    async fetch(request) {
      ...
      if (url.pathname === '/senecompare/index.html') {
        return new Response('<!doctype html><html><head><link rel="stylesheet" href="/styles.css?v=4.0.0"></head><body><main>SeneCompare AI</main><script src="/app.js?v=4.0.0"></script></body></html>', {
          status: 200,
          headers: { 'content-type': 'text/html; charset=utf-8' },
        });
      }
      if (url.pathname === '/senecompare/styles.css') {
        return new Response('body{font-family:system-ui}', { status: 200, headers: { 'content-type': 'text/css' } });
      }
      if (url.pathname === '/senecompare/app.js') {
        return new Response("const VERSION = '4.0.0'; globalThis.__SC_VERSION__ = VERSION;", {
          status: 200,
...
globalThis.fetch = async (input) => {
  ...
  if (url.pathname.endsWith('/senecompare-gateway/health')) {
    return new Response(JSON.stringify({
      ...
      status: 200,
      headers: { 'content-type': 'application/json', 'x-senecompare-version': '4.1.0' },
    });
  }
...
try {
  {
    const response = await application.fetch(new Request('https://senecompare.dakarstyle.com/?v=410', {
      headers: { accept: 'text/html' },
    }), env);
    assert.equal(response.status, 200);
    assert.equal(response.headers.get('x-senecompare-version'), '4.1.0');
    assert.equal(response.headers.get('x-senecompare-frontend'), 'cloudflare-local-assets');
    assert.match(response.headers.get('content-security-policy') || '', /default-src 'self'/);
    const html = await response.text();
    assert.match(html, /SeneCompare AI/);
    assert.match(html, /Version 4\.1\.0/);
    assert.match(html, /styles\.css\?v=4\.1\.0/);
    assert.match(html, /app\.js\?v=4\.1\.0/);
    assert.doesNotMatch(html, /4\.0\.0/);
  }

  {
    const response = await application.fetch(new Request('https://senecompare.dakarstyle.com/styles.css?v=4.1.0'), env);
    assert.equal(response.status, 200);
  ...
  {
    const response = await application.fetch(new Request('https://senecompare.dakarstyle.com/app.js?v=4.1.0'), env);
    assert.equal(response.status, 200);
    assert.match(response.headers.get('content-type') || '', /javascript/);
    const script = await response.text();
    assert.match(script, /4\.1\.0/);
    assert.doesNotMatch(script, /4\.0\.0/);
  }
...

  {
    ...
    assert.equal(payload.start_url, '/?source=pwa');
  }
  ...
  console.log('SeneCompare resilient frontend contract tests passed');
} finally {
