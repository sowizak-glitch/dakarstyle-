...
const OUTPUT = process.env.E2E_OUTPUT_DIR || 'test-results/senecompare-v541';
await fs.mkdir(OUTPUT, { recursive: true });

function cloudflareNoise(text) {
...
async function journey(browser, name, options) {
  ...
  const root = await page.goto(`${APP_URL}/?v=541&certification=1`, { waitUntil: 'domcontentloaded', timeout: 60_000 });
  assert.equal(root?.status(), 200, `${name}: root status`);
  assert.equal(root?.headers()['x-senecompare-release'], '5.4.1', `${name}: release header`);
  await page.waitForSelector('#scSponsored', { state: 'visible', timeout: 30_000 });
  assert.equal(await page.locator('script[src="/media-path-v541.js?v=541"]').count(), 1, `${name}: media path script`);

  await showCampaign(page, 'samabusiness-launch');
  const sama = page.locator('.sc-sponsored-visual img');
  await page.waitForFunction(() => document.querySelector('.sc-sponsored-visual img')?.src.includes('/media/v541/samabusiness-campaign.webp'));
  assert.match(await sama.getAttribute('src') || '', /^\/media\/v541\/samabusiness-campaign\.webp$/);
  ...
  await page.waitForFunction(() => document.querySelector('.sc-sponsored-visual img')?.src.includes('/media/v541/sowhat-africa-campaign.webp'));
  assert.match(await sowhat.getAttribute('src') || '', /^\/media\/v541\/sowhat-africa-campaign\.webp$/);
  assert.ok(await sowhat.evaluate((image) => image.complete && image.naturalWidth >= 300), `${name}: Sowhat visual`);
  assert.match(await page.locator('.sc-sponsored-label').innerText(), /Sponsorisé/i);
  ...
  const admin = await page.goto(`${APP_URL}/admin?v=541`, { waitUntil: 'domcontentloaded', timeout: 60_000 });
  ...
  const relevant = consoleErrors.filter((entry) => !cloudflareNoise(entry) && !/favicon|manifest|ResizeObserver/i.test(entry));
  assert.deepEqual(relevant, [], `${name}: app console errors`);
  await context.close();
...
try {
  const results = [
    await journey(browser, 'desktop', { viewport: { width: 1440, height: 1000 } }),
    await journey(browser, 'samsung', { ...devices['Galaxy S9+'] }),
  ];
  await fs.writeFile(path.join(OUTPUT, 'report.json'), JSON.stringify({ ok: true, release: '5.4.1', results }, null, 2));
  console.log(JSON.stringify({ ok: true, release: '5.4.1', results }, null, 2));
} finally {
  await browser.close();
}
