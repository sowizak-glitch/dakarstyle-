...
const ALLOWED = new Set([
  'samabusiness-campaign',
  'sowhat-africa-campaign'
]);
function serviceKey() {
...
function headers(type = 'application/json; charset=utf-8', etag = '') {
...
Deno.serve(async (request)=>{
  ...
  if (url.pathname.endsWith('/health')) return json({
    ok: true,
    version: VERSION,
    assets: [
      ...ALLOWED
  ...
  if (![
    'GET',
    'HEAD'
  ].includes(request.method)) return json({
    ok: false,
    code: 'METHOD_NOT_ALLOWED'
  }, 405);
  const file = url.pathname.split('/').filter(Boolean).at(-1) || '';
  const slug = file.replace(/\.(webp|jpg|jpeg|png)$/i, '');
  if (!ALLOWED.has(slug)) return json({
    ok: false,
    code: 'MEDIA_NOT_FOUND'
  }, 404);
  try {
    const { data, error } = await client().from('senecompare_media_assets').select('mime_type,content_base64,sha256,width,height,updated_at').eq('slug', slug).maybeSingle();
    if (error || !data?.content_base64) return json({
      ok: false,
    ...
    if (etag && request.headers.get('if-none-match') === `"${etag}"`) {
      return new Response(null, {
        status: 304,
        headers: headers(data.mime_type, etag)
      });
    }
