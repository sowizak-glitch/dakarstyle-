...
const db = SUPABASE_URL && SERVICE_KEY ? createClient(SUPABASE_URL, SERVICE_KEY, {
  auth: {
...
async function salesWorkspace(req, body) {
  ...
  const [customers, sales, orders, deliveries, products, expenses, cashMovements] = await Promise.all([
    ...
    db.from("sama_expenses").select("id,client_ref,category,label,amount,payment_method,scope,related_order_id,notes,happened_at,created_at,updated_at").eq("merchant_id", ctx.merchant.id).order("happened_at", {
      ascending: false
    }).limit(activityLimit),
  ...
  return {
    ...
    capabilities: {
      ...
      whatsapp_inbox_context: true,
      finance_context: true,
      low_stock_thresholds: true,
      guide_v17: true
    }
  };
}
async function copilotGuide(req, body, action) {
...
async function createSaleV2(req, body) {
...
async function saveCustomer(req, body) {
  const ctx = await sessionContext(req, true);
  ...
  if (body.name !== undefined) {
    ...
    if (!name) fail("Indiquez le nom du client.");
    patch.name = name;
  }
...
async function customerDetail(req, body) {
  const ctx = await sessionContext(req);
  ...
  const [customer, sales, orders] = await Promise.all([
    ...
    db.from("sama_sales").select("id,description,total_amount,paid_amount,remaining_amount,source,happened_at,payment_method,order_id").eq("merchant_id", ctx.merchant.id).eq("customer_id", customerId).is("deleted_at", null).order("happened_at", {
      ascending: false
    }).limit(200),
...
async function setOrderState(req, body) {
  const ctx = await sessionContext(req, true);
  ...
  if (requested === "delivered") {
    orderPatch.delivery_status = "delivered";
    orderPatch.delivered_at = now;
  }
...
Deno.serve(async (req)=>{
  ...
  if (req.method === "GET") return Response.json({
    ok: true,
    service: "samabusiness-api-v10",
    version: VERSION,
    upstream: "sama-business-api",
    sales_ops: true,
    copilot_context: true,
    guide_v17: true
  }, {
  ...
  if (localActions.has(action)) {
    try {
      const result = action === "sales_ops_workspace" ? await salesWorkspace(req, body) : action === "sales_ops_create_sale" ? await createSaleV2(req, body) : action === "sales_ops_save_customer" ? await saveCustomer(req, body) : action === "sales_ops_customer_detail" ? await customerDetail(req, body) : action === "sales_ops_set_order_state" ? await setOrderState(req, body) : action === "copilot_snapshot" ? await copilotGuide(req, body, "snapshot") : action === "copilot_interpret" ? await copilotGuide(req, body, "interpret_text") : await copilotGuide(req, body, "save_settings");
      return Response.json(result, {
        headers: cors(origin)
      });
    } catch (unknownError) {
      const error = unknownError;
      console.error("samabusiness-api-v10-sales-ops", {
