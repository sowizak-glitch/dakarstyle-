...
const CORE_PARTS = Array.from({
  length: 10
}, (_, index)=>`https://raw.githubusercontent.com/sowizak-glitch/dakarstyle-/${CORE_SOURCE_REF}/supabase/functions/samabusiness-field-ux/parts/part-${String(index).padStart(2, "0")}.js`);
...
const PARTS = [
  ...CORE_PARTS,
  SALES_PART,
  WHATSAPP_BUSINESS_PART,
  WHATSAPP_DIRECT_PART,
  ICON_PART,
  ICON_POLISH_PART,
  CLIENT_RECAP_PART,
  DESIGN_PART,
  COPILOT_PART,
  CAPTURE_MARKETING_PART,
  OPERATIONAL_PART,
  GUIDE_BRIDGE_PART,
  SALES_RECEIPT_BRIDGE_PART
];
const STUDIO = "https://xmdpmtvieqgoorbxytey.supabase.co/functions/v1/samabusiness-site-studio-ui?v=11.2.2";
let cached = "";
function publicDemoGuard() {
...
function stabilizeVoiceRecognition(code) {
  const startMarker = "      let finalText = area?.value || '';";
  const endMarker = "      recognition.onerror = (event) => {";
  const start = code.indexOf(startMarker);
  const end = start >= 0 ? code.indexOf(endMarker, start) : -1;
  if (start < 0 || end < 0) throw new Error("VOICE_RECOGNITION_SOURCE_NOT_FOUND");
  const replacement = [
    "      const baseText = (area?.value || '').trim();",
    "      const finalByIndex = new Map();",
    "      const mergeVoice = (base, phrase) => {",
    "        const left = String(base || '').trim();",
    "        const right = String(phrase || '').trim();",
    "        if (!right) return left;",
    "        if (!left) return right;",
    "        const a = left.split(/\\s+/);",
    "        const b = right.split(/\\s+/);",
    "        let overlap = 0;",
    "        for (let n = Math.min(12, a.length, b.length); n > 0; n--) {",
    "          const tail = a.slice(-n).join(' ').toLocaleLowerCase();",
    "          const head = b.slice(0, n).join(' ').toLocaleLowerCase();",
    ...
    "          if (!phrase) continue;",
    "          if (event.results[index].isFinal) finalByIndex.set(index, phrase);",
    "          else interim = mergeVoice(interim, phrase);",
    "        }",
    "        let combined = baseText;",
    "        [...finalByIndex.entries()].sort((a,b) => a[0] - b[0]).forEach(([,phrase]) => { combined = mergeVoice(combined, phrase); });",
    "        if (interim) combined = mergeVoice(combined, interim);",
    "        if (area) area.value = combined.trim();",
    "      };"
  ].join("\n");
...
async function source() {
  ...
  if (code.length < 160000 || !markers.every((marker)=>code.includes(marker))) throw new Error("FIELD_SOURCE_INVALID");
  code = code.replace("const VERSION = '10.2.0';", "const VERSION = '11.8.8';").replace("const VERSION='10.2.0';", "const VERSION='11.8.8';");
  code = stabilizeVoiceRecognition(code);
  if (!code.includes("const finalByIndex = new Map()") || !code.includes("const mergeVoice = (base, phrase)")) throw new Error("VOICE_STABILITY_PATCH_FAILED");
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    return new Response(req.method === "HEAD" ? null : code, {
      headers
    });
  } catch (error) {
