...
function serviceClient() {
  ...
  if (!key) throw Object.assign(new Error('SERVICE_UNAVAILABLE'), {
    status: 503
  });
...
function authClient(token = '') {
  const key = anonKey();
  if (!key) throw Object.assign(new Error('AUTH_UNAVAILABLE'), {
    status: 503
  });
  return createClient(PROJECT_URL, key, {
    auth: {
...
async function readJson(request, maxBytes = 16_384) {
  if (!(request.headers.get('content-type') || '').toLowerCase().includes('application/json')) throw Object.assign(new Error('JSON_REQUIRED'), {
    status: 415
  });
  const declared = Number(request.headers.get('content-length') || 0);
  if (declared > maxBytes) throw Object.assign(new Error('PAYLOAD_TOO_LARGE'), {
    status: 413
  });
...
async function requireAdmin(request) {
  ...
  if (!token) throw Object.assign(new Error('AUTH_REQUIRED'), {
    status: 401
  });
  const { data, error } = await authClient(token).auth.getUser(token);
  if (error || !data.user?.id || !data.user.email) throw Object.assign(new Error('AUTH_INVALID'), {
    status: 401
  });
  const { data: admin, error: adminError } = await serviceClient().from('senecompare_admin_users').select('user_id,email,role,active').eq('user_id', data.user.id).eq('active', true).maybeSingle();
  if (adminError || !admin || admin.email !== data.user.email.toLowerCase()) throw Object.assign(new Error('ADMIN_FORBIDDEN'), {
    status: 403
  });
  return admin;
}
async function publicAds(request) {
  const now = new Date().toISOString();
  const { data, error } = await serviceClient().from('senecompare_ad_campaigns').select('id,slug,brand,badge_fr,badge_wo,title_fr,title_wo,description_fr,description_wo,cta_fr,cta_wo,destination_url,image_url,creative,priority').eq('active', true).or(`starts_at.is.null,starts_at.lte.${now}`).or(`ends_at.is.null,ends_at.gt.${now}`).order('priority', {
    ascending: false
  }).limit(8);
  if (error) throw Object.assign(new Error('ADS_UNAVAILABLE'), {
    status: 503
  });
  return json(request, {
    ok: true,
...
async function trackEvent(request) {
  if (!await rateAllowed(request, 'senecompare_analytics_v53', 360)) return json(request, {
    ok: false,
    code: 'RATE_LIMITED'
  }, 429, {
    'Retry-After': '60'
  ...
  if (!PUBLIC_EVENTS.has(eventType)) return json(request, {
    ok: false,
    code: 'EVENT_INVALID'
  }, 400);
  const visitorId = clean(input.visitor_id, 120);
  const sessionId = clean(input.session_id, 120);
  if (visitorId.length < 8 || sessionId.length < 8) return json(request, {
    ok: false,
  ...
  const device = [
    ...
    'tablet',
    'desktop'
  ].includes(deviceRaw) ? deviceRaw : 'other';
  ...
  if (error) throw Object.assign(new Error('TRACK_FAILED'), {
    status: 503
  });
  return json(request, {
    ok: true
...
async function createLead(request) {
  if (!await rateAllowed(request, 'senecompare_partner_lead_v53', 8)) return json(request, {
    ok: false,
    code: 'RATE_LIMITED'
  }, 429, {
    'Retry-After': '60'
  ...
  const visitorId = clean(input.visitor_id, 120);
  if (businessName.length < 2 || contactName.length < 2 || !email || visitorId.length < 8) return json(request, {
    ok: false,
    code: 'LEAD_FIELDS_REQUIRED'
  }, 400);
  ...
  if (error) throw Object.assign(new Error('LEAD_WRITE_FAILED'), {
    status: 503
  });
  return json(request, {
    ok: true,
    id: data.id,
    message: 'Demande reçue. SeneCompare vous contactera.'
  }, 201);
}
async function requestAdminLogin(request) {
  if (!await rateAllowed(request, 'senecompare_admin_login_v53', 5)) return json(request, {
    ok: false,
    code: 'RATE_LIMITED'
  }, 429, {
    'Retry-After': '60'
  ...
  if (!email || email !== OWNER_EMAIL || !admin) return json(request, {
    ok: true,
    message: 'Si cette adresse est autorisée, un lien a été envoyé.'
  });
  const { error } = await authClient().auth.signInWithOtp({
    email,
    options: {
      shouldCreateUser: false,
      emailRedirectTo: `${PROD}/admin`
    }
  });
  if (error) throw Object.assign(new Error('LOGIN_EMAIL_FAILED'), {
    status: 503
  });
  return json(request, {
    ok: true,
    message: 'Lien sécurisé envoyé à votre adresse.'
  });
}
async function adminOverview(request, url) {
  const admin = await requireAdmin(request);
  const days = Math.max(1, Math.min(Number(url.searchParams.get('days') || 30), 365));
  const { data, error } = await serviceClient().rpc('senecompare_admin_overview', {
    p_days: days
  });
  if (error) throw Object.assign(new Error('OVERVIEW_FAILED'), {
    status: 503
  });
  return json(request, {
    ok: true,
...
async function updateCampaign(request) {
  ...
  if (error) throw Object.assign(new Error('CAMPAIGN_UPDATE_FAILED'), {
    status: 503
  });
  return json(request, {
    ok: true,
...
async function updateLead(request) {
  ...
  if (!id || !LEAD_STATUSES.has(status)) return json(request, {
    ok: false,
    code: 'LEAD_UPDATE_INVALID'
  }, 400);
  const { data, error } = await serviceClient().from('senecompare_partner_leads').update({
    status,
    updated_at: new Date().toISOString()
  }).eq('id', id).select('id,status,updated_at').single();
  if (error) throw Object.assign(new Error('LEAD_UPDATE_FAILED'), {
    status: 503
  });
  return json(request, {
    ok: true,
...
async function exportLeads(request) {
  await requireAdmin(request);
  const { data, error } = await serviceClient().from('senecompare_partner_leads').select('created_at,business_name,contact_name,email,phone,placement,status,message').order('created_at', {
    ascending: false
  }).limit(5000);
  if (error) throw Object.assign(new Error('EXPORT_FAILED'), {
    status: 503
  });
  const rows = [
    [
...
Deno.serve(async (request)=>{
  ...
  if (request.method === 'OPTIONS') {
    if (!originAllowed(request)) return new Response(null, {
      status: 403,
  ...
  if (!originAllowed(request)) return json(request, {
    ok: false,
    code: 'ORIGIN_FORBIDDEN'
  }, 403);
  try {
    if (currentRoute === 'health' && request.method === 'GET') return json(request, {
      ok: true,
      service: 'SeneCompare Admin & Ads',
      version: VERSION,
      owner_configured: true,
      privacy_mode: 'no_raw_ip'
    });
    ...
    const code = clean(error?.message || 'INTERNAL_ERROR', 80);
    console.error(JSON.stringify({
      event: 'senecompare_admin_v53_error',
      route: currentRoute,
      code,
      status
    }));
    return json(request, {
      ok: false,
      code: status >= 500 ? 'SERVICE_UNAVAILABLE' : code
    }, status);
  }
});
