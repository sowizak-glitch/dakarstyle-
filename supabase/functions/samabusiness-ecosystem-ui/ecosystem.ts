export default function ecosystem() {
  'use strict';
  if (window.__SAMA_ECOSYSTEM_UI_V1203__) return;
  window.__SAMA_ECOSYSTEM_UI_V1203__ = true;
...
.eco-fab{position:fixed;left:14px;bottom:calc(82px + env(safe-area-inset-bottom));z-index:880;border:0;border-radius:20px;min-height:57px;padding:9px 15px;background:linear-gradient(135deg,#071a32,#087a45);color:#fff;display:flex;align-items:center;gap:10px;font:900 13px inherit;box-shadow:0 18px 44px rgba(7,26,50,.35);cursor:pointer}.eco-fab span:first-child{width:38px;height:38px;border-radius:14px;background:rgba(255,255,255,.15);display:grid;place-items:center;font-size:20px}
...
.eco-card:focus-visible,.eco-fab:focus-visible,.eco-tool:focus-visible{outline:4px solid rgba(228,174,53,.55);outline-offset:2px}.eco-bottom{position:fixed;left:50%;transform:translateX(-50%);bottom:calc(8px + env(safe-area-inset-bottom));z-index:870;width:min(570px,calc(100% - 18px));display:grid;grid-template-columns:repeat(5,1fr);gap:5px;padding:7px;background:rgba(255,255,255,.93);backdrop-filter:blur(18px);border:1px solid rgba(16,35,28,.11);border-radius:21px;box-shadow:0 17px 45px rgba(7,26,50,.2)}.eco-bottom button{border:0;background:transparent;border-radius:14px;min-height:50px;font:850 10px inherit;color:#52615b;cursor:pointer}.eco-bottom button b{display:block;font-size:19px;margin-bottom:2px}.eco-bottom button:hover{background:#eaf8f1;color:#087a45}
...
`;
  ...
  function isVisible(el) { const r = el.getBoundingClientRect(); const style = getComputedStyle(el); return r.width > 0 && r.height > 0 && style.visibility !== 'hidden' && style.display !== 'none' && style.pointerEvents !== 'none'; }
  ...
  function externalModalOpen() {
    return [...document.querySelectorAll('[role="dialog"],.modal,.sheet,.drawer,[id$="Modal"],[id$="-modal"]')]
      .some((el) => !el.closest('#sama-eco-root') && el.getAttribute('aria-hidden') !== 'true' && !el.classList.contains('hidden') && isVisible(el));
  }
  ...
  function mount() {
    ...
    new MutationObserver(syncLayering).observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style','hidden','aria-hidden']});
    window.SAMABUSINESS = Object.assign(window.SAMABUSINESS || {}, { ecosystemUI: { version:'12.0.3', open, lang:() => state.lang } });
  }
