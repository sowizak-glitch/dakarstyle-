...
async function sessionMerchant(req) {
  ...
  const sessionQ = await db.from("sama_sessions").select("account_id,expires_at,revoked_at").eq("token_hash", tokenHash).maybeSingle();
  if (sessionQ.error) throw sessionQ.error;
  const session = sessionQ.data;
  if (!session || session.revoked_at || new Date(session.expires_at).getTime() <= Date.now()) {
    fail("Votre session a expiré. Reconnectez-vous.", 401);
  }
  const accountQ = await db.from("sama_accounts").select("id,is_active,subscription_status,suspended_at").eq("id", session.account_id).maybeSingle();
...
async function saveSupplier(req, body) {
  const ctx = await sessionMerchant(req);
  const productId = uuid(body.productId);
  if (!productId) fail("Produit invalide.");
  const productQ = await db.from("sama_products").select("id,name,metadata").eq("id", productId).eq("merchant_id", ctx.merchantId).maybeSingle();
  ...
  const reorderQuantity = Number.isFinite(parsedQuantity) && parsedQuantity > 0 ? Math.min(Math.round(parsedQuantity * 100) / 100, 100000) : 1;
  ...
  const updateQ = await db.from("sama_products").update({
    metadata,
    updated_at: new Date().toISOString()
  }).eq("id", productId).eq("merchant_id", ctx.merchantId).select("id,name,metadata").single();
  if (updateQ.error) throw updateQ.error;
  return reply(req, {
    ok: true,
    product: updateQ.data
  });
}
Deno.serve(async (req)=>{
  ...
  if (req.method === "GET") return reply(req, {
    ok: true,
    service: "samabusiness-supplier-api",
    version: VERSION
  });
