...
const PRIORITY_INTENTS = [
  [
    'artisanat',
    /plombier|electricien|mecanicien|menuisier|peintre|macon|reparation|depannage|nettoyage|lavage|artisan|couturier|tailleur|broderie|imprimerie/i
  ],
  [
    'sante',
    /clinique|medecin|pharmacie|dentiste|laboratoire|sante|opticien|radiologie|consultation/i
  ],
  [
    'education',
    /ecole|formation|cours|certification|universite|institut|apprentissage|enseignant/i
  ],
  [
    'finance',
    /assurance|mutuelle|credit|banque|pret|transfert d argent|change devise|microfinance/i
  ],
  [
    'livraison',
    /livraison|livrer|coursier|colis|expedition|messagerie|logistique|demenagement|yobbu|yobbante/i
  ],
  [
    'transport',
    /transport|taxi|vtc|bus|car rapide|clando|chauffeur|navette|trajet|transfert aeroport|dem dikk/i
  ],
  [
    'restauration',
    /pizza|restaurant|repas|fast[ -]?food|burger|sandwich|grill|traiteur|menu|thieb|ceebu|yassa|cafe|patisserie|boulangerie|nourriture|manger|lekk/i
  ],
  [
    'coiffure',
    /coiff|salon de beaut|barber|barbier|tresse|natte|manucure|pedicure|onglerie|spa/i
  ],
  [
    'immobilier',
    /appartement|studio|villa|terrain|immobilier|location maison|location bureau|chambre a louer|logement/i
  ],
  [
    'voyage',
    /hotel|auberge|hebergement|voyage|billet d avion|\bvol\b|agence de voyage|sejour/i
  ]
];
function serviceKey() {
...
async function synthesizeSpeech(request) {
  if (!openAiKey()) return json(request, {
    ok: false,
    code: 'VOICE_UNAVAILABLE'
  }, 503, {
    'Retry-After': '60'
  });
  if (!await rateAllowed(request, 'senecompare_speech_v51', 20)) return json(request, {
    ok: false,
    code: 'RATE_LIMITED'
  }, 429, {
    'Retry-After': '60'
  ...
  if (text.length < 2) return json(request, {
    ok: false,
    code: 'TEXT_REQUIRED'
  }, 400);
  ...
  if (!upstream.ok) return json(request, {
    ok: false,
...
async function transcribeAudio(request) {
  ...
  if (!await rateAllowed(request, 'senecompare_transcribe_v51', 20)) return json(request, {
    ok: false,
    code: 'RATE_LIMITED'
  }, 429, {
    'Retry-After': '60'
  ...
  const declared = Number(request.headers.get('content-length') || 0);
  if (declared > MAX_AUDIO_BYTES + 200_000) return json(request, {
    ok: false,
    code: 'AUDIO_TOO_LARGE'
  }, 413);
  const form = await request.formData();
  const audio = form.get('audio') || form.get('file');
  if (!(audio instanceof File) || audio.size < 100 || audio.size > MAX_AUDIO_BYTES) return json(request, {
    ok: false,
    code: 'AUDIO_REQUIRED'
  }, 400);
...
Deno.serve(async (request)=>{
  ...
  try {
    ...
    if (currentRoute === 'search' && request.method === 'POST') {
      ...
      if (query.length < 2 || query.length > 320) return json(request, {
        ok: false,
        code: 'QUERY_INVALID'
      }, 400);
      if (!await rateAllowed(request, 'hybrid_search_gateway_v51', 40)) return json(request, {
        ok: false,
        code: 'RATE_LIMITED'
      }, 429, {
        'Retry-After': '60'
    ...
    if (currentRoute === 'feedback' && request.method === 'POST') {
      if (!await rateAllowed(request, 'feedback_gateway_v51', 60)) return json(request, {
        ok: false,
        code: 'RATE_LIMITED'
      }, 429);
      const input = await readJson(request, MAX_EVENT_BYTES);
    ...
    if (currentRoute === 'alerts' && request.method === 'POST') {
      if (!await rateAllowed(request, 'alert_gateway_v51', 20)) return json(request, {
        ok: false,
        code: 'RATE_LIMITED'
      }, 429);
      const input = await readJson(request, MAX_EVENT_BYTES);
      ...
      if (!payload.offer_id && !payload.query) return json(request, {
        ok: false,
        code: 'OFFER_OR_QUERY_REQUIRED'
      }, 400);
      if (!payload.phone && !payload.email) return json(request, {
        ok: false,
    ...
    if (currentRoute === 'merchant' && request.method === 'POST') {
      if (!await rateAllowed(request, 'merchant_gateway_v51', 12)) return json(request, {
        ok: false,
        code: 'RATE_LIMITED'
      }, 429);
      const input = await readJson(request, MAX_EVENT_BYTES);
      ...
      if (!payload.business_name || !payload.phone) return json(request, {
        ok: false,
    ...
    if (![
      'GET',
      'POST'
    ].includes(request.method)) return json(request, {
      ok: false,
    ...
    if ([
      ...
      413,
      415
    ].includes(status)) return json(request, {
      ok: false,
      code
    }, status);
    if (status === 503) return json(request, {
      ok: false,
